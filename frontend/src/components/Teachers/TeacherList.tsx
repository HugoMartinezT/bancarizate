...
const TeacherList = () => {
  ...
  const loadTeachers = async (page = 1, search = '', status = 'all') => {
    try {
      ...
      const response = await apiService.getTeachers({
        ...
        sortOrder: 'desc'  // Manteniendo el parámetro de la versión funcional
      });
  ...
  const getStatusIcon = (status: Teacher['status']) => {
    switch (status) {
      ...
      case 'retired': return <Sparkles className="w-3 h-3 text-blue-500" />;
      default: return null;
    }
  };
                ...
                return (
                  <tr key={teacher.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-xs text-gray-700 min-w-[180px]">
                      {teacher.courses.length > 0 ? teacher.courses.join(', ') : 'Sin cursos asignados'}
                    </td>
