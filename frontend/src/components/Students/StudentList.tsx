...
const StudentList = () => {
  ...
  const getStatusIcon = (status: Student['status']) => {
    switch (status) {
      ...
      case 'graduated': return <GraduationCap className="w-3.5 h-3.5 text-blue-500" />;
      default: return null;
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Lista de Estudiantes</h1>
            <p className="text-sm text-gray-500 mt-1">Gestiona estudiantes en el sistema</p>
          </div>
          <button
        ...
        <div className="bg-white rounded-lg shadow border border-gray-100 overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Estudiante</th>
                ...
                <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-2">
                    <div className="flex flex-col">
                      <span className="font-medium text-gray-900 text-sm">{student.firstName} {student.lastName}</span>
                      <span className="text-xs text-gray-500">{student.run}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2 text-sm text-gray-700">{student.institution}</td>
                  <td className="px-4 py-2 text-sm text-gray-700">{student.course}</td>
                  <td className="px-4 py-2 text-sm text-gray-700">{formatDate(student.createdAt)}</td>
